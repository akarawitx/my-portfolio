  ...
  <header class="header" :class="{ scrolled: isScrolled }">
    <div class="nav-inner">
      ...
      <ul class="nav-links">
        ...
        <li><RouterLink to="/projects" @click="closeMenu">Project</RouterLink></li>
...
.nav-logo { display: flex; align-items: center; transition: filter 0.2s; }
.nav-logo:hover { filter: drop-shadow(0 0 8px var(--accent)); }
...
.hamburger span.open:nth-child(1) { transform: translateY(7px) rotate(45deg); background: var(--accent); }
.hamburger span.open:nth-child(2) { opacity: 0; }
.hamburger span.open:nth-child(3) { transform: translateY(-7px) rotate(-45deg); background: var(--accent); }
...
.mobile-menu ul { list-style: none; padding: 16px 20px 24px; display: flex; flex-direction: column; gap: 4px; }
.mobile-menu a {
  font-family: var(--font-display);
...
@media (max-width: 1024px) { .nav-inner { padding: 0 32px; } .nav-links { gap: 32px; } }
...
@media (max-width: 480px) { .nav-inner { padding: 0 16px; } .header { padding: 14px 0; } }
</style>